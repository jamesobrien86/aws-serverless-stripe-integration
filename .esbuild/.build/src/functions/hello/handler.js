...
var require_core = __commonJS({
  "node_modules/@middy/core/index.js"(exports, module2) {
    var middy2 = (baseHandler = () => {
      ...
      var _a;
      (_a = plugin == null ? void 0 : plugin.beforePrefetch) == null ? void 0 : _a.call(plugin);
      const beforeMiddlewares = [];
      const afterMiddlewares = [];
      const onErrorMiddlewares = [];
      const instance = (event = {}, context = {}) => {
        var _a2;
        (_a2 = plugin == null ? void 0 : plugin.requestStart) == null ? void 0 : _a2.call(plugin);
        const request = {
          event,
          context,
          response: void 0,
    ...
    var runRequest = async (request, beforeMiddlewares, baseHandler, afterMiddlewares, onErrorMiddlewares, plugin) => {
      ...
      try {
        ...
        if (request.response === void 0) {
          (_a = plugin == null ? void 0 : plugin.beforeHandler) == null ? void 0 : _a.call(plugin);
          request.response = await baseHandler(request.event, request.context);
          (_b = plugin == null ? void 0 : plugin.afterHandler) == null ? void 0 : _b.call(plugin);
        ...
        await ((_c = plugin == null ? void 0 : plugin.requestEnd) == null ? void 0 : _c.call(plugin, request));
      }
    ...
    var runMiddlewares = async (request, middlewares, plugin) => {
      ...
      for (const nextMiddleware of middlewares) {
        (_a = plugin == null ? void 0 : plugin.beforeMiddleware) == null ? void 0 : _a.call(plugin, nextMiddleware == null ? void 0 : nextMiddleware.name);
        const res = await (nextMiddleware == null ? void 0 : nextMiddleware(request));
        (_b = plugin == null ? void 0 : plugin.afterMiddleware) == null ? void 0 : _b.call(plugin, nextMiddleware == null ? void 0 : nextMiddleware.name);
        if (res !== void 0) {
  ...
  "node_modules/@middy/util/index.js"(exports, module2) {
    ...
    var createClient = async (options, request) => {
      ...
      if (options.awsClientAssumeRole) {
        ...
        awsClientCredentials = await getInternal({ credentials: options.awsClientAssumeRole }, request);
      }
    ...
    var canPrefetch = (options) => {
      return !(options == null ? void 0 : options.awsClientAssumeRole) && !(options == null ? void 0 : options.disablePrefetch);
    };
    var getInternal = async (variables, request) => {
      ...
      for (const internalKey of values) {
        ...
        if (typeof (valuePromise == null ? void 0 : valuePromise.then) !== "function") {
          valuePromise = Promise.resolve(valuePromise);
        }
        promises.push(valuePromise.then((value) => pathOptionKey.reduce((p, c) => p == null ? void 0 : p[c], value)));
      }
      values = await Promise.allSettled(promises);
      const errors = values.filter((res) => res.status === "rejected").map((res) => res.reason.message);
      if (errors.length)
        throw new Error(JSON.stringify(errors));
      return keys.reduce((obj, key, index) => __spreadProps(__spreadValues({}, obj), { [sanitizeKey(key)]: values[index].value }), {});
    };
    ...
    var processCache = (options, fetch = () => void 0, request) => {
      ...
      if (cacheExpiry) {
        ...
        if (unexpired) {
          return __spreadProps(__spreadValues({}, cached), { cache: true });
        }
      }
    ...
    var modifyCache = (cacheKey, value) => {
      if (!cache[cacheKey])
        return;
      cache[cacheKey] = __spreadProps(__spreadValues({}, cache[cacheKey]), { value, modified: true });
    };
    ...
    var normalizeHttpResponse = (response) => {
      ...
      response.headers = (response == null ? void 0 : response.headers) ?? {};
      return response;
    };
    ...
    var createError = (code, message, properties = {}) => {
      ...
      function HttpError(message2) {
        const msg = message2 ?? statuses[code];
  ...
  "node_modules/@middy/http-json-body-parser/index.js"(exports, module2) {
    ...
    var httpJsonBodyParserMiddleware = (opts = {}) => {
      ...
      const httpJsonBodyParserMiddlewareBefore = async (request) => {
        ...
        const contentTypeHeader = (headers == null ? void 0 : headers["Content-Type"]) ?? (headers == null ? void 0 : headers["content-type"]);
